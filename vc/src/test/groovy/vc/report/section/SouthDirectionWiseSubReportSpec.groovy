...
class SouthDirectionWiseSubReportSpec extends Specification{
	...
	def "File with 8 readings, with sequence A->A->A->A->A->A->A->A yield 4 North Vehicles"(int countForSession,int countSouthBoundVehicles){
		given:
			...
			sessionWiseMainReport = new SouthDirectionWiseSubReport(sessionWiseMainReport)
			def sessionReport = "\t\tTotal:" + countForSession +"\n";
			def directionReport = "SOUTH" + "\t\tTotal:" + countSouthBoundVehicles + "\n";
		...
		where:
			countForSession|countSouthBoundVehicles
			4			   |0
	}
	
	def "File with 6 readings, with sequence A->B->A->B->A->A yield 1 SOUTH and 1 NORTH Vehicles"(int countForSession,int countSouthBoundVehicles){
		given:
	...
	def "File with 8 readings, with sequence A->B->A->B->A->B->A->B yield 2 South Vehicles"(int countForSession,int countSouthBoundVehicles){
		given:
